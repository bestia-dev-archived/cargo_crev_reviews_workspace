...
//! it calculates it and replace the next token: it can be a text_noe, attribute, a complex node,...
//! It can work on the server and on the client
//! the main difference is in the naming of template variables
//! on the server they start with "s" like "st_name" : st_, sb_,
//! on the web web browser (client) they start with "w" like "wt_name": wt_, wb,
//! So it is possible to use partially on the server and partially on the client.
...
pub fn render<T: HtmlServerTemplateRender>(self_0: &T, html_template_raw: &str, server_or_client_prefixes: ServerOrClient) -> String {
    ...
    let nodes = unwrap!(render_template_raw_to_nodes(self_0, &html_template_raw, HtmlOrSvg::Html, "", 0, &prefixes));
                    ...
                    Token::Attribute(name, value) => {
                        if retain_this_node == true {
                            if name.starts_with(&prefixes.attr_text) {
                                ...
                                let repl_txt = self_0.replace_with_string(placeholder, subtemplate_name, row_number);
                                ...
                                let placeholder = name.trim_start_matches("data-");
                                let repl_url = self_0.replace_with_url(placeholder, subtemplate_name, row_number);
                                replace_attr_name = Some(s!(value));
                                replace_attr_repl_name = Some(s!(name));
                    ...
                    Token::Comment(txt) => {
                        if retain_this_node == true {
                            ...
                            if txt.starts_with(&prefixes.text) {
                                let repl_txt = self_0.replace_with_string(txt, subtemplate_name, row_number);
                                replace_string = Some(repl_txt);
                            } else if txt.starts_with(&prefixes.url) {
                                let repl_url = self_0.replace_with_url(txt, subtemplate_name, row_number);
                                replace_url = Some(repl_url);
                            } else if txt.starts_with(&prefixes.exist) {
                                // boolean if this is true than render the next node, else don't render
                                retain_next_node_or_attribute = self_0.retain_next_node_or_attribute(txt);
                                ...
                                let repl_vec_nodes = self_0.render_sub_template(template_name, sub_templates, prefixes);
