...
//! 3. the browser imports the wasm module and starts the init function that requests `srv_review_list`. This responds with: method_name, extract_html and data.
//! 4. wasm (inside the browser) is rust code. First it matches method_name and calls the appropriate function. It processes the extract_html with the data and inserts it into index.html (the empty shell).
